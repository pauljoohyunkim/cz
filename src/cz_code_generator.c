...
static LLVMValueRef cz_code_generator_generate_lvalue_function_call(CZ_CodeGenerator* cg, const CZ_Environment* env, const CZ_Environment_Backend* env_b, const CZ_AST_Node* node);
...
static int cz_code_generator_generate_return_statement(CZ_CodeGenerator* cg, const CZ_Environment* env, const CZ_Environment_Backend* env_b, const CZ_AST_Node* node) {
    ...
    if (cg->current_function_return->kind == CZ_TYPE_KIND_REFERENCE) {
        LLVMValueRef llvm_return_val_ref = cz_code_generator_generate_lvalue(cg, env, env_b, node->return_statement.expression);
...
static int cz_code_generator_generate_while_statement(CZ_CodeGenerator* cg, const CZ_Environment* env, const CZ_Environment_Backend* env_b, const CZ_AST_Node* node) {
    ...
    NULL_POINTER_ERROR_HANDLE(env_b);
    NULL_POINTER_ERROR_HANDLE(node);
    INVALID_NODE_TYPE_ERROR_HANDLE(node, CZ_AST_WhileStatementNodeType);

    LLVMValueRef llvm_cur_func = LLVMGetBasicBlockParent(LLVMGetInsertBlock(cg->builder));

    // Condition, body, end block
    LLVMBasicBlockRef llvm_cond_block = LLVMAppendBasicBlockInContext(cg->ctx, llvm_cur_func, "while.cond");
    LLVMBasicBlockRef llvm_body_block = LLVMAppendBasicBlockInContext(cg->ctx, llvm_cur_func, "while.body");
    LLVMBasicBlockRef llvm_end_block = LLVMAppendBasicBlockInContext(cg->ctx, llvm_cur_func, "while.end");

    // Current position branches to condition block
    LLVMBuildBr(cg->builder, llvm_cond_block);

    // Set builder to write to condition block.
    LLVMPositionBuilderAtEnd(cg->builder, llvm_cond_block);
    LLVMValueRef llvm_cond_val = cz_code_generator_generate_expr(cg, env, env_b, node->while_statement.condition, false);
    NULL_POINTER_ERROR_HANDLE(llvm_cond_val);

    // If condition is true, go to body again.
    LLVMBuildCondBr(cg->builder, llvm_cond_val, llvm_body_block, llvm_end_block);
    ...
    // If there was no return, go back to condition block.
    if (LLVMGetBasicBlockTerminator(LLVMGetInsertBlock(cg->builder)) == NULL) {
        LLVMBuildBr(cg->builder, llvm_cond_block);
    }
...
static int cz_code_generator_generate_block_statement(CZ_CodeGenerator* cg, const CZ_Environment* env, const CZ_Environment_Backend* env_b, const CZ_AST_Node* node) {
    ...
    NULL_POINTER_ERROR_HANDLE(env);
    NULL_POINTER_ERROR_HANDLE(env_b);
    INVALID_NODE_TYPE_ERROR_HANDLE(node, CZ_AST_BlockStatementNodeType);

    inner_env_b = cz_environment_backend_enter_scope(env_b);
    NULL_POINTER_ERROR_HANDLE(inner_env_b);

    unsigned int inner_stmt_count = node->statement_list.statement_count;
...
static LLVMValueRef cz_code_generator_generate_lvalue(CZ_CodeGenerator* cg, const CZ_Environment* env, const CZ_Environment_Backend* env_b, const CZ_AST_Node* node) {
    ...
    switch (node->node_type) {
        ...
        case CZ_AST_FunctionCallNodeType:
            llvm_val = cz_code_generator_generate_lvalue_function_call(cg, env, env_b, node);
            break;
        default:
            goto error_cleanup;
    }
...
static LLVMValueRef cz_code_generator_generate_lvalue_function_call(CZ_CodeGenerator* cg, const CZ_Environment* env, const CZ_Environment_Backend* env_b, const CZ_AST_Node* node) {
    ...
    LLVMValueRef llvm_callee_func = cz_code_generator_generate_expr(cg, env, env_b, node->function_call.callee, false);
    LLVMTypeRef llvm_callee_func_type = cz_environment_backend_lookup_type(cg, node->function_call.callee->decoration->resolved_type);

    unsigned int param_count = node->function_call.callee->decoration->resolved_type->function.param_count;
    llvm_args = (LLVMValueRef*) calloc(param_count, sizeof(LLVMValueRef));
    ...
    for (unsigned int i = 0; i < param_count; i++) {
        ...
        llvm_args[i] =
            param_type->kind == CZ_TYPE_KIND_REFERENCE ?
                                cz_code_generator_generate_lvalue(cg, env, env_b, node->function_call.arguments[i]) :
                                cz_code_generator_generate_expr(cg, env, env_b, node->function_call.arguments[i], false);
    }

    LLVMValueRef llvm_func_call = LLVMBuildCall2(cg->builder, llvm_callee_func_type, llvm_callee_func, llvm_args, param_count, "func_ref_ret");
        ...
        case CZ_AST_IdentifierNodeType:
            {
                ...
                if (node->decoration->resolved_type->kind == CZ_TYPE_KIND_FUNCTION) {
                    llvm_val = llvm_identifier_ref;
                } else {
    ...
    switch (node->binary_expression.op) {
        ...
        case CZ_TT_SLASH:
            if (llvm_lhs_type_kind == LLVMFloatTypeKind && llvm_rhs_type_kind == LLVMFloatTypeKind) {
                ...
                llvm_val =
                    node->binary_expression.left->decoration->resolved_type->kind == CZ_AST_TYPE_KIND_UINT32 ? LLVMBuildUDiv(cg->builder, llvm_lhs, llvm_rhs, "udivtmp")
        ...
        case CZ_TT_PERCENT:
            if (llvm_lhs_type_kind == LLVMIntegerTypeKind && llvm_rhs_type_kind == LLVMIntegerTypeKind) {
                llvm_val =
                    node->binary_expression.left->decoration->resolved_type->kind == CZ_AST_TYPE_KIND_UINT32 ? LLVMBuildURem(cg->builder, llvm_lhs, llvm_rhs, "uremtmp")
        ...
        case CZ_TT_GREATER:
            if (llvm_lhs_type_kind == LLVMFloatTypeKind && llvm_rhs_type_kind == LLVMFloatTypeKind) {
                ...
                llvm_val = LLVMBuildICmp(cg->builder,
                                         node->binary_expression.left->decoration->resolved_type->kind == CZ_AST_TYPE_KIND_UINT32 ? LLVMIntUGT : LLVMIntSGT,
                                         llvm_lhs,
                                         llvm_rhs,
                                         "gttmp");
            }
        ...
        case CZ_TT_LESS:
            if (llvm_lhs_type_kind == LLVMFloatTypeKind && llvm_rhs_type_kind == LLVMFloatTypeKind) {
                ...
                llvm_val = LLVMBuildICmp(cg->builder,
                                         node->binary_expression.left->decoration->resolved_type->kind == CZ_AST_TYPE_KIND_UINT32 ? LLVMIntULT : LLVMIntSLT,
                                         llvm_lhs,
                                         llvm_rhs,
                                         "lttmp");
            }
        ...
        case CZ_TT_GREATER_EQUAL:
            if (llvm_lhs_type_kind == LLVMFloatTypeKind && llvm_rhs_type_kind == LLVMFloatTypeKind) {
                ...
                llvm_val = LLVMBuildICmp(cg->builder,
                                         node->binary_expression.left->decoration->resolved_type->kind == CZ_AST_TYPE_KIND_UINT32 ? LLVMIntUGE : LLVMIntSGE,
                                         llvm_lhs,
                                         llvm_rhs,
                                         "getmp");
            }
        ...
        case CZ_TT_LESS_EQUAL:
            if (llvm_lhs_type_kind == LLVMFloatTypeKind && llvm_rhs_type_kind == LLVMFloatTypeKind) {
                ...
                llvm_val = LLVMBuildICmp(cg->builder,
                                         node->binary_expression.left->decoration->resolved_type->kind == CZ_AST_TYPE_KIND_UINT32 ? LLVMIntULE : LLVMIntSLE,
                                         llvm_lhs,
                                         llvm_rhs,
                                         "letmp");
            }
        ...
        case CZ_TT_NUMERICAL_LITERAL:
            {
                if (strchr(node->literal.lexeme, '.') != NULL) {
                    ...
                    // Determine sign and choose appropriate parsing function & sign extension flag
                    bool is_unsigned = (strchr(node->literal.lexeme, 'u') != NULL);

                    uint64_t value = is_unsigned
                                         ? strtoul(node->literal.lexeme, NULL, 10)
                                         : (uint64_t)strtol(node->literal.lexeme, NULL, 10);

                    llvm_val = LLVMConstInt(
                        LLVMInt32TypeInContext(cg->ctx),
                        value,
                        !is_unsigned  // true for signed, false for unsigned
                    );
                }
            }
            break;
