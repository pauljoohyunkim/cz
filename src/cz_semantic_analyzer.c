...
static int cz_semantic_analyzer_check_function_call_expression(CZ_SemanticAnalyzer* sa, CZ_Environment* env, CZ_AST_Node* expr) {
    ...
    // 1. Check callee and see if it a function (will support things like (f(3))(2))
    ...
    for (unsigned int i = 0; i < param_count; i++) {
        ...
        // 4.2 If parameter is reference
        if (param_type->kind == CZ_TYPE_KIND_REFERENCE) {
            // 4.2.1 Argument must be l-value.
            ...
            // 4.2.2 If argument is const, then param type must not be mutable.
    ...
    decor = cz_ast_decoration_create(expr->function_call.callee->decoration->resolved_type->function.return_type,
        CZ_VALUE_CATEGORY_RVALUE,
        false,
        expr->function_call.callee->decoration->resolved_type->function.return_type->kind != CZ_TYPE_KIND_REFERENCE,
        env->scope_level);
    if (decor == NULL) {
